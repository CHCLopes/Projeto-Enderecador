...
export const DeclarationLabel: React.FC<Props> = ({ sender, recipient }) => {
  ...
  // Formatador para o Total do Rodapé (Com R$)
  const formatMoney = (val: number) => 
    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(val);

  // NOVO: Formatador para a Tabela (Sem R$)
    ...
    <div className="w-full h-full border-2 border-black p-3 flex flex-col bg-white text-[10px] font-sans leading-tight relative box-border print:border-l-2 print:border-t-2">
      ...
      <div className="grid grid-cols-2 gap-4 border-b border-black pb-2 mb-2">
        <div>
            ...
            <span className="block font-bold leading-tight mb-0.5 line-clamp-2 overflow-hidden">{sender.nome}</span>
            <div className="leading-tight text-[10px]">
                <span className="block">{sender.logradouro}, {sender.numero} {sender.complemento}</span>
                <span className="block">{sender.bairro} - {sender.localidade}/{sender.uf}</span>
                <span className="block">CEP: {sender.cep}</span>
            </div>
        </div>
        <div>
            <span className="font-bold block text-[10px]">DESTINATÁRIO:</span>
            <span className="block font-bold leading-tight mb-0.5 line-clamp-2 overflow-hidden">{recipient.nome}</span>
            <div className="leading-tight text-[10px]">
                <span className="block">{recipient.logradouro}, {recipient.numero} {recipient.complemento}</span>
                <span className="block">{recipient.bairro} - {recipient.localidade}/{recipient.uf}</span>
                <span className="block">CEP: {recipient.cep}</span>
            </div>
        </div>
      </div>
